...
const sendPasswordResetEmail = async (data: PasswordResetEmailJob["data"]) => {
	const templateData = {
		...
		supportEmail: "support@insightcast.com",
		companyName: "insightcast",
	};

	await Mail.sendHtmlEmail(data.email, "Reset Your Password - insightcast", "passwordReset", templateData);
};
...
const sendWelcomeEmail = async (data: WelcomeEmailJob["data"]) => {
	...
	await Mail.sendHtmlEmail(data.email, "Welcome to insightcast!", "welcome", templateData);
};
